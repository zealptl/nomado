...
export default async function handler(req: any, res: any) {
  ...
  if (!isOwner) return res.status(403).json({ error: 'Only the trip owner can refresh invite links' });
  ...
  return res.status(200).json({ token: newToken, url: buildInviteUrl(req, newToken) });
}
